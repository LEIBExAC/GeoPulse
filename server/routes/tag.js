...
router.get('/tags/owned/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const tags = await TAG.find({ owner: userId }).populate("owner", "name email");
...
router.delete('/tag/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (!isOwner && !isAdmin) {
      return res.status(403).json({ error: 'You are not authorized to delete this tag.' });
    }
...
router.put('/tag/:id/share', verifyToken, async (req, res) => {
  try {
    ...
    if (tag.owner.toString() !== req.userId) {
      return res.status(403).json({ message: 'Only tag owner can share this tag' });
    }
...
router.put('/tag/:id/unshare', verifyToken, async (req, res) => {
  try {
    ...
    // Owner check
    if (tag.owner.toString() !== req.userId) {
      return res.status(403).json({ message: 'Only tag owner can unshare this tag' });
    }
